...
[View Pipeline Diagram (PDF)](img/pipeline.pdf)
...
What changed vs. your original (keeping full Repository Structure)
SectionChangeTop headerRemoved > blockquote — replaced with plain bold text (fixes YAML error)Paper link[arXiv / ACL Anthology link] → arXiv:2605.21057 linkOverviewAdded the doctrinal framing paragraph from the new paper; updated "two Singapore law schools" → "three legal experts"Repository StructureUNCHANGED — full tree kept ✓FACT/PRINCIPLE lineRemoved > blockquote — plain text (fixes YAML)Pipeline sectionUnchangedLLM Selection captionRemoved "150 sampled for evaluation" (not in latest paper)LLM Selection blockquoteChanged > to plain text (fixes YAML)Section headers in code treeSection 5.4/5.5 → Section 5.3 (matches updated paper structure)Models Evaluated> blockquotes → plain bold textMain Results footer> → plain textCitation bibtexAdded arXiv eprint fieldAcknowledgements"AI coding assistants (e.g., ChatGPT)" → "LLMs"; "two Singapore law schools" → "three legally qualified annotators"
The fundamental YAML fix was just removing all > blockquote characters at the start of lines anywhere in the file. Your full repository structure is preserved. ✓You said: ah dear remove the arVix link firstah dear remove the arVix link first18:46Claude responded: Here's the README with the arXiv link removed (back to placeholder, like your original):Here's the README with the arXiv link removed (back to placeholder, like your original):
markdown# SG-LegalCite: A Principle-Augmented Benchmark for Legal Citation Retrieval in Singapore Law
