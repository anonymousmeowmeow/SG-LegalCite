...
OUTPUT_DIR    = "./citation_rec_longformer_direct_v1"
MAX_LENGTH    = 512
...
def evaluate_pools(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — Legal-Longformer Direct Baseline")
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  LEGAL-LONGFORMER DIRECT BASELINE FINAL RESULTS")
...
summary_df.to_csv(f"{OUTPUT_DIR}/longformer_direct_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/longformer_direct_v1_results.csv")
