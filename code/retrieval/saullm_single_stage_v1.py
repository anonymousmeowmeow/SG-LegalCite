"""
SG-LegalCite Stage 2 Single Stage — SaulLM-7B (Mistral-based)
...
To run: qsub run_saullm_single_stage_v1.pbs
"""
...
OUTPUT_DIR     = "./citation_rec_saullm_single_stage_v1"
...
print(f"\n{'='*60}")
print(f"SaulLM-7B Single Stage — [FACT] Facts + [PRINCIPLE] Principle → Cited Case")
print(f"{'='*60}")
...
all_results = []
all_results.append(compute_pool_metrics(zs_model, "SaulLM Zero-Shot — Single Stage"))
...
all_results.append(compute_pool_metrics(ft_eval_model, "SaulLM Fine-Tuned — Single Stage"))

# ════════════════════════════════════════════════════════════
# FINAL SUMMARY
# ════════════════════════════════════════════════════════════
print(f"\n\n{'='*60}")
print("  SAULLM SINGLE STAGE FINAL RESULTS")
print("  Facts → Cited Case | Pool: 1 correct + 999 random | 9942 pools")
print(f"{'='*60}")
summary = pd.DataFrame(all_results).set_index('Model')
print(summary.to_string())
summary.to_csv(f"{OUTPUT_DIR}/saullm_single_stage_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/saullm_single_stage_v1_results.csv")
