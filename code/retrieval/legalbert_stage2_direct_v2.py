# ============================================================
# SG-LegalCite Stage 2 Direct Baseline V2
...
# Fair comparison with V3 (both have 1 correct case per pool)
# ============================================================
...
OUTPUT_DIR    = "./citation_rec_stage2_direct_v2"
MAX_LENGTH    = 512
BATCH_SIZE    = 64
EPOCHS        = 10
LEARNING_RATE = 2e-5
...
# ── LOAD POOLS ────────────────────────────────────────────────
print("\nLoading direct V2 candidate pools...")
...
# ── DATASET ───────────────────────────────────────────────────
class Stage2DirectV2Dataset(Dataset):
    """Training: Facts → Cited Case (no key principles)"""
    def __init__(self, df, tokenizer, max_length=512):
        # Deduplicate to unique (fact, cited_case) pairs for training
        self.pairs      = df[['fact', 'cited_case']].drop_duplicates().reset_index(drop=True)
        self.tokenizer  = tokenizer
        self.max_length = max_length
        print(f"  Training pairs (unique fact-case): {len(self.pairs)}")
    ...
    def __getitem__(self, idx):
        ...
        query_enc = self.tokenizer(
            row['fact'], truncation=True, max_length=self.max_length,
            padding='max_length', return_tensors='pt'
        )
        case_enc  = self.tokenizer(
            row['cited_case'], truncation=True, max_length=self.max_length,
            padding='max_length', return_tensors='pt'
        )
        return {
...
# ── EVALUATION FUNCTION ───────────────────────────────────────
def evaluate_direct_v2(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — Direct Baseline V2 (Facts → Case)")
    print(f"  Pool: 1 correct + 999 random | 9942 pools")
    print(f"{'='*60}")

    pool_ids = list(pools.keys())

    # Encode all unique facts
    ...
    # Encode all unique case candidates
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 1: Zero-Shot Legal BERT — Direct Baseline V2")
print("="*60)
...
all_results = []
result      = evaluate_direct_v2(zs_model, tokenizer, label="Zero-Shot")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 2: Fine-Tuning — Facts → Cited Case directly")
print("="*60)

train_dataset = Stage2DirectV2Dataset(train_df, tokenizer, MAX_LENGTH)
val_dataset   = Stage2DirectV2Dataset(val_df,   tokenizer, MAX_LENGTH)
...
for epoch in range(EPOCHS):
    ...
    if avg_val < best_val_loss:
        ...
        torch.save(save_model.state_dict(), f"{OUTPUT_DIR}/stage2_direct_v2_model.pt")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 3: Fine-Tuned Legal BERT — Direct Baseline V2")
print("="*60)

eval_model = LegalBERTDualEncoder(MODEL_NAME).to(device)
state_dict = torch.load(f"{OUTPUT_DIR}/stage2_direct_v2_model.pt", map_location=device)
...
result = evaluate_direct_v2(eval_model, tokenizer, label="Fine-Tuned")
all_results.append(result)
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  STAGE 2 DIRECT BASELINE V2 FINAL RESULTS")
print("  Zero-Shot vs Fine-Tuned | Facts → Case (no principles)")
print("  Pool: 1 correct + 999 random | 9942 pools")
print("="*60)
summary_df = pd.DataFrame(all_results).set_index('Model')
print(summary_df.to_string())
summary_df.to_csv(f"{OUTPUT_DIR}/stage2_direct_v2_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/stage2_direct_v2_results.csv")
