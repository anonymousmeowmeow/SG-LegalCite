"""
SG-LegalCite Stage 2 Single Stage — AdaptLLM law-LLM
...
Training setup IDENTICAL to all encoder models and AdaptLLM Direct Baseline:
  BATCH=64, EPOCHS=10, LR=2e-5, WARMUP=0.1
  Temperature: learnable, init=0.07
  Save best by val_loss
  pin_memory=True, num_workers=4
  Host: aimc-gna2, 7x A100 GPUs
...
Only difference from adaptllm_direct_v1.py:
  Query: [FACT] {fact} [PRINCIPLE] {principle}  (was: fact only)
  Pool:  stage2_single_stage_pools.json (9979)  (was: stage2_direct_candidate_pools_v2.json 9942)

To run: qsub run_adaptllm_single_stage_v2.pbs
"""
...
POOL_PATH      = "./stage2_single_stage_pools.json"       # 9979 pools
LOOKUP_PATH    = "./stage2_case_lookup.json"
OUTPUT_DIR     = "./citation_rec_adaptllm_single_stage_v2"
...
print(f"\n{'='*60}")
print(f"AdaptLLM law-LLM Single Stage")
...
def load_base_model():
    ...
    print(f"  Loading {MODEL_NAME} with {ATTN_IMPLEMENTATION}...")
    t0 = time.time()
    try:
        ...
        print(f"  ⚠️ {ATTN_IMPLEMENTATION} failed ({e}), falling back to eager")
        m = AutoModel.from_pretrained(MODEL_NAME, quantization_config=bnb,
                                      device_map="auto", trust_remote_code=True,
                                      torch_dtype=torch.bfloat16,
...
zs_base  = get_peft_model(zs_base, lora_cfg)
zs_model = AdaptLLMDualEncoder(zs_base, embedding_dim=EMBEDDING_DIM, temp_init=TEMP_INIT)
print("✓ Zero-shot model ready")

all_results = []
all_results.append(compute_pool_metrics(zs_model, "AdaptLLM Zero-Shot — Single Stage"))
...
ft_base  = get_peft_model(ft_base, LoraConfig(
    ...
    target_modules=["q_proj","k_proj","v_proj","o_proj",
                    "gate_proj","up_proj","down_proj"]))
model = AdaptLLMDualEncoder(ft_base, embedding_dim=EMBEDDING_DIM, temp_init=TEMP_INIT)
...
ft_eval_model = AdaptLLMDualEncoder(ft_eval_base, embedding_dim=EMBEDDING_DIM, temp_init=TEMP_INIT)
...
all_results.append(compute_pool_metrics(ft_eval_model, "AdaptLLM Fine-Tuned — Single Stage"))

# ════════════════════════════════════════════════════════════
# FINAL SUMMARY
# ════════════════════════════════════════════════════════════
print(f"\n\n{'='*60}")
print("  ADAPTLLM SINGLE STAGE FINAL RESULTS")
print("  [FACT] Facts + [PRINCIPLE] Principle → Cited Case")
print("  Pool: 1 correct + 999 random | 9979 pools")
print(f"{'='*60}")
summary = pd.DataFrame(all_results).set_index('Model')
print(summary.to_string())
summary.to_csv(f"{OUTPUT_DIR}/adaptllm_single_stage_v2_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/adaptllm_single_stage_v2_results.csv")
