...
OUTPUT_DIR    = "./citation_rec_pileoflaw_single_stage_v2"
MAX_LENGTH    = 512
...
def evaluate_pools(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — Pile-of-Law BERT Single Stage")
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  PILE-OF-LAW BERT SINGLE STAGE FINAL RESULTS")
...
summary_df.to_csv(f"{OUTPUT_DIR}/pileoflaw_single_stage_v2_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/pileoflaw_single_stage_v2_results.csv")
