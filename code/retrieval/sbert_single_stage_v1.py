...
subprocess.check_call([sys.executable, '-m', 'pip', 'install',
                       'sentence-transformers', 'numpy<2.0', '-q'])
...
OUTPUT_DIR    = "./citation_rec_sbert_single_stage"
BATCH_SIZE    = 64
EPOCHS        = 10
LEARNING_RATE = 2e-5
WARMUP_RATIO  = 0.1
...
def evaluate_direct_v2(model, label):
    print(f"\n{'='*60}")
    print(f"  {label} — SBERT Single Stage (Fact + GT Principle → Case)")
    print(f"  Pool: 1 correct + 999 random | 9979 pools (same as Legal BERT Single Stage)")
    print(f"{'='*60}")
...
train_pairs = train_df[['fact', 'principle', 'cited_case']].drop_duplicates().reset_index(drop=True)
print(f"  Training pairs: {len(train_pairs)}")

train_examples   = [InputExample(texts=[f"[FACT] {row['fact']} [PRINCIPLE] {row['principle']}", row['cited_case']])
...
ft_model.fit(
    ...
    output_path=f"{OUTPUT_DIR}/sbert_single_stage_model",
    show_progress_bar=True,
    save_best_model=True,
)
print(f"\n✓ Training complete. Model saved to {OUTPUT_DIR}/sbert_single_stage_model")
...
eval_model = SentenceTransformer(f"{OUTPUT_DIR}/sbert_single_stage_model", device=device)
...
print("  Zero-Shot vs Fine-Tuned | Fact + Ground Truth Principle → Case")
print("  Pool: 1 correct + 999 random | 9979 pools (same as Legal BERT Single Stage)")
print("="*60)
summary_df = pd.DataFrame(all_results).set_index('Model')
print(summary_df.to_string())
summary_df.to_csv(f"{OUTPUT_DIR}/sbert_single_stage_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/sbert_single_stage_results.csv")
