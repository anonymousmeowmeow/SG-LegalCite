...
OUTPUT_DIR    = "./citation_rec_customlegalbert_direct_v1"
MAX_LENGTH    = 512
BATCH_SIZE    = 64
EPOCHS        = 10
LEARNING_RATE = 2e-5
...
def evaluate_pools(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — Custom Legal-BERT Direct Baseline")
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  CUSTOM LEGAL-BERT DIRECT BASELINE FINAL RESULTS")
...
summary_df.to_csv(f"{OUTPUT_DIR}/customlegalbert_direct_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/customlegalbert_direct_v1_results.csv")
