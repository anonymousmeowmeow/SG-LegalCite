# ============================================================
# SG-LegalCite Stage 2 Direct Baseline — Custom Legal-BERT
# Task: [FACT] Fact + [PRINCIPLE] Ground Truth Principle → Cited Case
# Model: casehold/custom-legalbert (37GB Harvard Law corpus)
# Pool: SAME as Legal BERT Single Stage — stage2_single_stage_pools.json
...
# ── CONFIG ────────────────────────────────────────────────────
MODEL_NAME    = "casehold/custom-legalbert"
POOL_PATH     = "./stage2_single_stage_pools.json"
LOOKUP_PATH   = "./stage2_case_lookup.json"
OUTPUT_DIR    = "./citation_rec_customlegalbert_single_stage_v1"
MAX_LENGTH    = 512
BATCH_SIZE    = 64
EPOCHS        = 10
LEARNING_RATE = 2e-5
...
# ── MODEL ─────────────────────────────────────────────────────
class CustomLegalBERTDualEncoder(nn.Module):
...
def evaluate_pools(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — Custom Legal-BERT Single Stage")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 1: Zero-Shot Custom Legal-BERT — Direct Baseline")
print("="*60)

tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
print(f"✓ Tokenizer loaded | Vocab size: {tokenizer.vocab_size}")

zs_model = CustomLegalBERTDualEncoder(MODEL_NAME, pooling='mean')
...
all_results = []
result = evaluate_pools(zs_model, tokenizer, "Custom Legal-BERT Zero-Shot")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 2: Fine-Tuning Custom Legal-BERT — Facts → Cited Case")
print("="*60)
...
ft_model = CustomLegalBERTDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 3: Fine-Tuned Custom Legal-BERT — Direct Baseline")
print("="*60)

eval_model = CustomLegalBERTDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
result = evaluate_pools(eval_model, tokenizer, "Custom Legal-BERT Fine-Tuned")
all_results.append(result)
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  CUSTOM LEGAL-BERT SINGLE STAGE FINAL RESULTS")
...
summary_df.to_csv(f"{OUTPUT_DIR}/customlegalbert_single_stage_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/customlegalbert_single_stage_v1_results.csv")
