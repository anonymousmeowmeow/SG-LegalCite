"""
SG-LegalCite Stage 2 Direct Baseline — SaulLM-7B (Mistral-based)
Task:   Extract of Facts → Cited Case
Pool:   stage2_direct_candidate_pools_v2.json (9942 pools)
Split:  By URL (same as ALL other models for fair comparison)

Training setup IDENTICAL to AdaptLLM and all encoder models:
...
SaulLM-specific (inherited from SaulLM7b_FineTuned_512_v3.py):
...
To run: qsub run_saullm_direct_v2.pbs
"""
...
OUTPUT_DIR     = "./citation_rec_saullm_direct_v2"
...
print(f"\n{'='*60}")
print(f"SaulLM-7B Direct Baseline — Facts → Cited Case")
print(f"{'='*60}")
print(f"Model:      {MODEL_NAME}")
print(f"Pool:       stage2_direct_candidate_pools_v2.json (9942)")
...
df_clean = df[['URL', 'Extract of Facts', 'Cited Case']].copy()
df_clean.columns = ['url', 'fact', 'cited_case']
df_clean = df_clean.dropna(subset=['fact', 'cited_case', 'url'])
...
class CitationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        q_enc = self.tokenizer(str(row['fact']),       truncation=True,
...
def compute_pool_metrics(model, label):
    ...
    fact_embs = encode_texts(unique_facts)
    ...
    for fid in pool_ids:
        ...
        sims       = np.dot(case_embs[rows], fact_embs[fact_to_idx[p['fact_text']]])
...
zs_model = SaulLMDualEncoder(zs_base, embedding_dim=EMBEDDING_DIM, temp_init=TEMP_INIT)
print("✓ Zero-shot model ready")

all_results = []
all_results.append(compute_pool_metrics(zs_model, "SaulLM Zero-Shot — Direct Baseline"))
...
all_results.append(compute_pool_metrics(ft_eval_model, "SaulLM Fine-Tuned — Direct Baseline"))

# ════════════════════════════════════════════════════════════
# FINAL SUMMARY
# ════════════════════════════════════════════════════════════
print(f"\n\n{'='*60}")
print("  SAULLM DIRECT BASELINE FINAL RESULTS")
print("  Facts → Cited Case | Pool: 1 correct + 999 random | 9942 pools")
print(f"{'='*60}")
summary = pd.DataFrame(all_results).set_index('Model')
print(summary.to_string())
summary.to_csv(f"{OUTPUT_DIR}/saullm_direct_v2_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/saullm_direct_v2_results.csv")
