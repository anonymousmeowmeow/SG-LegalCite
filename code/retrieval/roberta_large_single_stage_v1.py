# ============================================================
# SG-LegalCite Stage 2 Single Stage — Legal-en-RoBERTa Large
# Task: [FACT] Fact + [PRINCIPLE] Ground Truth Principle → Cited Case
# Model: joelniklaus/legal-english-roberta-large (RoBERTa-Large, 337M params)
# Architecture: 24 layers, 1024 hidden
# Pre-training: 292GB+ MultiLegalPile (US, EU, UK, Canada)
# Reference: Niklaus et al. (2024) "MultiLegalPile: A 689GB Multilingual Legal Corpus"
...
# ── CONFIG — identical to Legal BERT / SBERT / Custom Legal-BERT / Longformer / Pile-of-Law / SAILER / RoBERTa Direct ──
MODEL_NAME    = "joelniklaus/legal-english-roberta-large"
POOL_PATH     = "./stage2_single_stage_pools.json"
LOOKUP_PATH   = "./stage2_case_lookup.json"
OUTPUT_DIR    = "./citation_rec_roberta_large_single_stage_v1"
MAX_LENGTH    = 512
...
print(f"\nModel: {MODEL_NAME}")
print(f"Architecture: RoBERTa-Large (24 layers, 1024 hidden, 337M params)")
print(f"Pre-training: 292GB+ MultiLegalPile (US, EU, UK, Canada)")
print(f"Batch: {BATCH_SIZE} | Epochs: {EPOCHS} | LR: {LEARNING_RATE} | Warmup: {WARMUP_RATIO}")
print(f"Query format: [FACT] Fact [PRINCIPLE] Ground Truth Principle")
print(f"Note: Same settings as Legal BERT / SBERT / Custom Legal-BERT / Legal-Longformer / Pile-of-Law / SAILER")
...
# ── LOAD POOLS ─────────────────────────────────────────────────
print("\nLoading single stage candidate pools (same as all models)...")
...
# ── MODEL ──────────────────────────────────────────────────────
class RoBERTaLargeDualEncoder(nn.Module):
    """Dual encoder using Legal-en-RoBERTa Large for citation recommendation.
    Uses AutoTokenizer + AutoModel (standard RoBERTa handling).
    Pre-trained on 292GB+ MultiLegalPile (US, EU, UK, Canada).
    """
    def __init__(self, model_name, pooling='mean'):
        super().__init__()
        self.encoder     = AutoModel.from_pretrained(model_name)
        self.pooling     = pooling
        self.temperature = nn.Parameter(torch.tensor(0.07))  # same as all models
...
def evaluate_pools(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — Legal-en-RoBERTa Large Single Stage")
    print(f"  Query: [FACT] Fact [PRINCIPLE] Principle | Pool: 1 correct + 999 random | 9979 pools")
    print(f"{'='*60}")

    pool_ids       = list(pools.keys())
    unique_queries = list(set(pools[pid]['query_text'] for pid in pool_ids))
    query_to_idx   = {q: i for i, q in enumerate(unique_queries)}

    print(f"  Encoding {len(unique_queries)} unique queries...")
    query_embs = encode_texts(model, tokenizer, unique_queries)
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 1: Zero-Shot Legal-en-RoBERTa Large — Single Stage")
print("="*60)

tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
print(f"✓ Tokenizer loaded | Vocab size: {tokenizer.vocab_size}")

zs_model = RoBERTaLargeDualEncoder(MODEL_NAME, pooling='mean')
...
all_results = []
result = evaluate_pools(zs_model, tokenizer, "RoBERTa-Large Zero-Shot")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 2: Fine-Tuning Legal-en-RoBERTa Large — [FACT] Fact [PRINCIPLE] Principle → Cited Case")
print("="*60)
...
val_loader    = DataLoader(val_dataset,   batch_size=BATCH_SIZE, shuffle=False,
                           num_workers=4, pin_memory=True)
print(f"  Train triples: {len(train_dataset)} | Val triples: {len(val_dataset)}")
print(f"  Train batches: {len(train_loader)} | Val batches: {len(val_loader)}")

ft_model = RoBERTaLargeDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 3: Fine-Tuned Legal-en-RoBERTa Large — Single Stage")
print("="*60)

eval_model = RoBERTaLargeDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
result = evaluate_pools(eval_model, tokenizer, "RoBERTa-Large Fine-Tuned")
all_results.append(result)
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  LEGAL-EN-ROBERTA LARGE SINGLE STAGE FINAL RESULTS")
print("  Zero-Shot vs Fine-Tuned | [FACT] Fact [PRINCIPLE] Principle → Case")
print("  Pool: 1 correct + 999 random | 9979 pools (same as all models)")
print("="*60)
summary_df = pd.DataFrame(all_results).set_index('Model')
print(summary_df.to_string())
summary_df.to_csv(f"{OUTPUT_DIR}/roberta_large_single_stage_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/roberta_large_single_stage_v1_results.csv")
