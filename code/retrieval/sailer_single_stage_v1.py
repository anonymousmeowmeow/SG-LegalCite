# ============================================================
# SG-LegalCite Stage 2 Single Stage — SAILER
# Task: [FACT] Fact + [PRINCIPLE] Ground Truth Principle → Cited Case
# Model: CSHaitao/SAILER_en (BERT-base, 110M params)
# Pre-training: 10M+ judgments (CN primary, US) with MLM + Structure-aware objectives
# Reference: Li et al. (2023) "SAILER: Structure-aware Pre-trained Language Model
#            for Legal Case Retrieval" (SIGIR'23)
...
# ── CONFIG — identical to Legal BERT / SBERT / Custom Legal-BERT / Longformer / Pile-of-Law ──
MODEL_NAME    = "CSHaitao/SAILER_en"
POOL_PATH     = "./stage2_single_stage_pools.json"
LOOKUP_PATH   = "./stage2_case_lookup.json"
OUTPUT_DIR    = "./citation_rec_sailer_single_stage_v1"
MAX_LENGTH    = 512
...
print(f"\nModel: {MODEL_NAME}")
print(f"Architecture: BERT-base (12 layers, 768 hidden, 110M params)")
print(f"Pre-training: 10M+ judgments (CN primary, US) with MLM + Structure-aware objectives")
...
# ── MODEL ──────────────────────────────────────────────────────
class SAILERDualEncoder(nn.Module):
    """Dual encoder using SAILER for citation recommendation.
    SAILER uses AutoTokenizer + AutoModel (standard, no special handling needed).
    Pre-trained with structure-aware objectives on 10M+ legal judgments.
    """
...
class SingleStageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Same query format as all other Single Stage models
...
def evaluate_pools(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — SAILER Single Stage")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 1: Zero-Shot SAILER — Single Stage")
print("="*60)

tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
print(f"✓ Tokenizer loaded | Vocab size: {tokenizer.vocab_size}")

zs_model = SAILERDualEncoder(MODEL_NAME, pooling='mean')
...
all_results = []
result = evaluate_pools(zs_model, tokenizer, "SAILER Zero-Shot")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 2: Fine-Tuning SAILER — [FACT] Fact [PRINCIPLE] Principle → Cited Case")
print("="*60)
...
ft_model = SAILERDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 3: Fine-Tuned SAILER — Single Stage")
print("="*60)

eval_model = SAILERDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
result = evaluate_pools(eval_model, tokenizer, "SAILER Fine-Tuned")
all_results.append(result)
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  SAILER SINGLE STAGE FINAL RESULTS")
...
summary_df.to_csv(f"{OUTPUT_DIR}/sailer_single_stage_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/sailer_single_stage_v1_results.csv")
