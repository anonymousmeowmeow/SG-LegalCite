# ============================================================
# SG-LegalCite Stage 2 Direct Baseline — Pile-of-Law BERT Large
# Task: Facts → Cited Case directly (no key principles)
# Model: pile-of-law/legalbert-large-1.7M-2 (BERT-Large, 340M params)
# Pre-training: 256GB Pile of Law (35 legal sources)
...
# ── CONFIG — identical to Legal BERT / SBERT / Custom Legal-BERT / Longformer ──
...
OUTPUT_DIR    = "./citation_rec_pileoflaw_direct_v1"
MAX_LENGTH    = 512
...
print(f"\nModel: {MODEL_NAME}")
print(f"Architecture: BERT-Large (24 layers, 1024 hidden, 340M params)")
print(f"Pre-training: 256GB Pile of Law (35 legal sources)")
...
# ── MODEL ──────────────────────────────────────────────────────
class PileOfLawDualEncoder(nn.Module):
    """Dual encoder using Pile-of-Law BERT Large.
    BERT-Large architecture: 24 layers, 1024 hidden, 340M params.
    Uses BertTokenizer + BertModel (standard BERT classes, same as Legal BERT).
    """
...
def evaluate_pools(model, tokenizer, label):
    print(f"\n{'='*60}")
    print(f"  {label} — Pile-of-Law BERT Direct Baseline")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 1: Zero-Shot Pile-of-Law BERT — Direct Baseline")
print("="*60)

tokenizer = BertTokenizer.from_pretrained(MODEL_NAME)
print(f"✓ Tokenizer loaded | Vocab size: {tokenizer.vocab_size}")

zs_model = PileOfLawDualEncoder(MODEL_NAME, pooling='mean')
...
all_results = []
result = evaluate_pools(zs_model, tokenizer, "Pile-of-Law BERT Zero-Shot")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 2: Fine-Tuning Pile-of-Law BERT — Facts → Cited Case")
print("="*60)
...
ft_model = PileOfLawDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 3: Fine-Tuned Pile-of-Law BERT — Direct Baseline")
print("="*60)

eval_model = PileOfLawDualEncoder(MODEL_NAME, pooling='mean')
if torch.cuda.device_count() > 1:
...
result = evaluate_pools(eval_model, tokenizer, "Pile-of-Law BERT Fine-Tuned")
all_results.append(result)
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  PILE-OF-LAW BERT DIRECT BASELINE FINAL RESULTS")
...
summary_df.to_csv(f"{OUTPUT_DIR}/pileoflaw_direct_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/pileoflaw_direct_v1_results.csv")
