"""
SG-LegalCite Stage 2 Direct Baseline — LawMA 8B
...
To run: qsub run_lawma_direct_v3.pbs
"""
...
OUTPUT_DIR        = "./citation_rec_lawma_direct_v3"
...
print(f"\n{'='*60}")
print(f"LawMA 8B Direct Baseline — Facts → Cited Case")
print(f"{'='*60}")
print(f"Model:       {MODEL_NAME}")
print(f"Task:        Extract of Facts → Cited Case")
...
def compute_pool_metrics(model, label):
    ...
    print(f"  Encoding {len(unique_facts)} facts...")
    fact_embs = encode_texts(unique_facts)
    print(f"  Encoding {len(case_texts)} cases...")
...
all_results = []
all_results.append(compute_pool_metrics(zs_model, "LawMA Zero-Shot — Direct Baseline"))
...
all_results.append(compute_pool_metrics(ft_eval_model, "LawMA Fine-Tuned — Direct Baseline"))

# ============================================================================
# FINAL SUMMARY
# ============================================================================
print(f"\n\n{'='*60}")
print("  LAWMA DIRECT BASELINE FINAL RESULTS")
print("  Facts → Cited Case | Pool: 1 correct + 999 random")
print(f"{'='*60}")
summary = pd.DataFrame(all_results).set_index('Model')
print(summary.to_string())
summary.to_csv(f"{OUTPUT_DIR}/lawma_direct_v3_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/lawma_direct_v3_results.csv")
