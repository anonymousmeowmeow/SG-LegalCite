...
Task:   [FACT] Extract of Facts + [ISSUE_GROUP] Issue Group + [ISSUE] Issue
        + [PRINCIPLE] Key Principle → Cited Case
...
Ablation purpose:
  Tests whether adding ALL structured fields (issue_group + issue + principle)
  to the query gives more discriminative signal than principle alone.
  Compared against fact-only and fact+principle baselines on identical pools.
...
To run: qsub run_saullm_all_fields_v17.pbs
"""
...
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'  # 2 A100 GPUs (v17 — per-user a100q cap is 2)
...
OUTPUT_DIR     = "./citation_rec_saullm_all_fields_v17"
...
print(f"\n{'='*60}")
print(f"SaulLM-7B All Fields v17 — [FACT] [ISSUE_GROUP] [ISSUE] [PRINCIPLE] → Cited Case")
print(f"{'='*60}")
print(f"Model:      {MODEL_NAME}")
print(f"Pool:        stage2_all_fields_pools_v3.json (9979)")
...
print(f"GPUs:       2x A100 (v17 — per-user a100q max is 2, was 4) (v10 — PYTHONPATH chains .local + shannon/packages)")
...
df_clean['query'] = ('[FACT] '         + df_clean['fact'].astype(str) +
                     ...
                     ' [ISSUE] '       + df_clean['issue'].astype(str) +
...
class CitationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Query: [FACT] fact [ISSUE_GROUP] issue_group [ISSUE] issue [PRINCIPLE] principle
...
def compute_pool_metrics(model, label):
    ...
    unique_queries  = list(set(
        ...
        ' [ISSUE] '       + pools[fid]['issue_text'] +
        ' [PRINCIPLE] '   + pools[fid]['principle_text']
        for fid in pool_ids
    ))
...
summary.to_csv(f"{OUTPUT_DIR}/saullm_all_fields_v17_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/saullm_all_fields_v17_results.csv")
