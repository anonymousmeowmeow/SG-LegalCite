# ============================================================
# SG-LegalCite All Fields Ablation — SBERT
# Task: [FACT] Fact + [ISSUE_GROUP] Issue Group + [ISSUE] Issue
#       + [PRINCIPLE] Ground Truth Principle → Cited Case
# Model: sentence-transformers/all-mpnet-base-v2
# Pool: stage2_all_fields_pools_v3.json (9979 pools)
# Zero-Shot + Fine-Tuned
# Purpose: Ablation to test whether adding ALL structured fields
#          improves over fact+principle alone (reviewer R3 critique)
# ============================================================
...
POOL_PATH     = "./stage2_all_fields_pools_v3.json"
LOOKUP_PATH   = "./stage2_case_lookup.json"
OUTPUT_DIR    = "./citation_rec_sbert_all_fields_v2"
BATCH_SIZE    = 64
EPOCHS        = 10
LEARNING_RATE = 2e-5
WARMUP_RATIO  = 0.1
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 1: Zero-Shot SBERT — All Fields Ablation")
print("="*60)
...
all_results = []
result      = evaluate_all_fields(zs_model, label="SBERT Zero-Shot — All Fields")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 2: Fine-Tuning SBERT — All Fields → Cited Case")
print("="*60)

train_pairs = train_df[['fact', 'issue_group', 'issue', 'principle', 'cited_case']].drop_duplicates().reset_index(drop=True)
print(f"  Training pairs: {len(train_pairs)}")

train_examples = [InputExample(texts=[
    f"[FACT] {row['fact']} [ISSUE_GROUP] {row['issue_group']} [ISSUE] {row['issue']} [PRINCIPLE] {row['principle']}",
    row['cited_case']
...
ft_model.fit(
    ...
    output_path=f"{OUTPUT_DIR}/sbert_all_fields_model",
    show_progress_bar=True,
    save_best_model=True,
)
print(f"\n✓ Training complete. Model saved to {OUTPUT_DIR}/sbert_all_fields_model")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 3: Fine-Tuned SBERT — All Fields Ablation")
print("="*60)

eval_model = SentenceTransformer(f"{OUTPUT_DIR}/sbert_all_fields_model", device=device)
print("✓ Fine-tuned SBERT model loaded")

result = evaluate_all_fields(eval_model, label="SBERT Fine-Tuned — All Fields")
all_results.append(result)
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  SBERT ALL FIELDS ABLATION FINAL RESULTS")
print("  Zero-Shot vs Fine-Tuned")
print("  Query: [FACT] [ISSUE_GROUP] [ISSUE] [PRINCIPLE] → Cited Case")
print("  Pool: 1 correct + 999 random | 9979 pools")
print("="*60)
summary_df = pd.DataFrame(all_results).set_index('Model')
print(summary_df.to_string())
summary_df.to_csv(f"{OUTPUT_DIR}/sbert_all_fields_v2_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/sbert_all_fields_v2_results.csv")
