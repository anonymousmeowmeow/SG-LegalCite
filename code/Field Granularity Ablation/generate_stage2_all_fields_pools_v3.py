# ============================================================
# SG-LegalCite All-Fields Ablation V3: Generate Candidate Pools
# Task: [FACT] Fact [ISSUE_GROUP] Issue Group [ISSUE] Issue [PRINCIPLE] Principle → Cited Case
# Pool: 1 correct case + 999 random negatives
# One pool per (fact, issue_group, issue, principle, cited_case) tuple
...
# ── STEP 4: Get unique (fact, issue_group, issue, principle, cited_case) tuples ──
print("\nBuilding (fact, issue_group, issue, principle, cited_case) tuples...")
tuples = test_df[['fact', 'issue_group', 'issue', 'principle', 'cited_case']].drop_duplicates().reset_index(drop=True)
print(f"Unique tuples: {len(tuples)}")

# Stats on how many cited cases per (fact, issue_group, issue, principle) combination
combo_to_cases = defaultdict(set)
for _, row in tuples.iterrows():
    combo_to_cases[(row['fact'], row['issue_group'], row['issue'], row['principle'])].add(row['cited_case'])
counts = [len(v) for v in combo_to_cases.values()]
print(f"Unique (fact, issue_group, issue, principle) combinations: {len(combo_to_cases)}")
print(f"Cited cases per combination — min: {min(counts)} | max: {max(counts)} | mean: {np.mean(counts):.2f}")
print(f"Combinations with 1 cited case:   {sum(1 for c in counts if c == 1)} ({100*sum(1 for c in counts if c == 1)/len(counts):.1f}%)")
print(f"Combinations with 2+ cited cases: {sum(1 for c in counts if c >= 2)} ({100*sum(1 for c in counts if c >= 2)/len(counts):.1f}%)")

# ── STEP 5: Generate pools ─────────────────────────────────────
print("\nGenerating all-fields V3 candidate pools...")
random.seed(RANDOM_SEED)
all_case_ids = set(range(len(all_cases)))
pools = {}

for pool_id, row in tuples.iterrows():
    ...
    pools[pool_id] = {
        "fact_text":         row['fact'],
        "issue_group_text":  row['issue_group'],
        "issue_text":        row['issue'],
        "principle_text":    row['principle'],
        "query_text":        f"[FACT] {row['fact']} [ISSUE_GROUP] {row['issue_group']} [ISSUE] {row['issue']} [PRINCIPLE] {row['principle']}",
...
# ── STEP 6: Save ──────────────────────────────────────────────
pool_path   = OUTPUT_DIR + "stage2_all_fields_pools_v3.json"
...
print(f"\n✓ Saved stage2_all_fields_pools_v3.json → {pool_path}")
...
print(f"\nDone! Use stage2_all_fields_pools_v3.json for all-fields ablation evaluation.")
