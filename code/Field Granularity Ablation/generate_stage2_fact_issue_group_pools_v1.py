# ============================================================
# SG-LegalCite Fact+Issue Group Ablation V1: Generate Candidate Pools
# Task: [FACT] Fact [ISSUE_GROUP] Issue Group → Cited Case
...
# Issue group text taken from first row matching each triple.
# Run on CLUSTER
# ============================================================
...
triples = triples[['fact', 'issue_group', 'principle', 'cited_case']]
print(f"Unique (fact, principle, cited_case) triples: {len(triples)}")

# ── STEP 5: Generate pools ─────────────────────────────────────
print("\nGenerating fact+issue_group candidate pools...")
random.seed(RANDOM_SEED)
...
for pool_id, row in triples.iterrows():
    ...
    pools[pool_id] = {
        ...
        "query_text":        f"[FACT] {row['fact']} [ISSUE_GROUP] {row['issue_group']}",
...
# ── STEP 6: Save ──────────────────────────────────────────────
pool_path   = OUTPUT_DIR + "stage2_fact_issue_group_pools_v1.json"
...
print(f"\n✓ Saved stage2_fact_issue_group_pools_v1.json → {pool_path}")
...
print(f"\nDone! Use stage2_fact_issue_group_pools_v1.json for fact+issue_group ablation evaluation.")
