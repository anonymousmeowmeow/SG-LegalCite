...
Task:   [FACT] Extract of Facts + [ISSUE] Issue
        → Cited Case
...
Ablation purpose:
  Tests whether Issue alone (without Principle) achieves similar retrieval
...
To run: qsub run_saullm_fact_issue_v1.pbs
"""
...
OUTPUT_DIR     = "./citation_rec_saullm_fact_issue_v1"
...
print(f"\n{'='*60}")
print(f"SaulLM-7B Fact+Issue v1 — [FACT] [ISSUE] → Cited Case")
print(f"{'='*60}")
print(f"Model:      {MODEL_NAME}")
print(f"Pool:        stage2_fact_issue_pools_v1.json (9979)")
...
def compute_pool_metrics(model, label):
    print(f"\n{'='*60}")
    print(f"  {label}")
    print(f"  Query: [FACT] fact [ISSUE] issue")
...
summary.to_csv(f"{OUTPUT_DIR}/saullm_fact_issue_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/saullm_fact_issue_v1_results.csv")
