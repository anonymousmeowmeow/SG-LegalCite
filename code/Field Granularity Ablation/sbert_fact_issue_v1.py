# ============================================================
# SG-LegalCite Fact+Issue Ablation — SBERT
# Task: [FACT] Fact + [ISSUE] Issue
#        → Cited Case
# Model: sentence-transformers/all-mpnet-base-v2
# Pool: stage2_fact_issue_pools_v1.json (9979 pools)
# Zero-Shot + Fine-Tuned
# Purpose: Ablation R3 — fact+issue vs fact+principle baseline
#          Tests if issue alone provides retrieval signal vs principle
# ============================================================
...
POOL_PATH     = "./stage2_fact_issue_pools_v1.json"
LOOKUP_PATH   = "./stage2_case_lookup.json"
OUTPUT_DIR    = "./citation_rec_sbert_fact_issue_v1"
BATCH_SIZE    = 64
EPOCHS        = 10
LEARNING_RATE = 2e-5
WARMUP_RATIO  = 0.1
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 1: Zero-Shot SBERT — Fact+Issue Ablation")
print("="*60)
...
all_results = []
result      = evaluate_all_fields(zs_model, label="SBERT Zero-Shot — Fact+Issue")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 2: Fine-Tuning SBERT — Fact+Issue → Cited Case")
print("="*60)
...
train_pairs = train_pairs[['fact', 'issue', 'principle', 'cited_case']]
print(f"  Training pairs: {len(train_pairs)}")

train_examples = [InputExample(texts=[
    f"[FACT] {row['fact']} [ISSUE] {row['issue']}",
    row['cited_case']
...
ft_model.fit(
    ...
    output_path=f"{OUTPUT_DIR}/sbert_fact_issue_model",
    show_progress_bar=True,
    save_best_model=True,
)
print(f"\n✓ Training complete. Model saved to {OUTPUT_DIR}/sbert_fact_issue_model")
...
# ════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  PART 3: Fine-Tuned SBERT — Fact+Issue Ablation")
print("="*60)

eval_model = SentenceTransformer(f"{OUTPUT_DIR}/sbert_fact_issue_model", device=device)
print("✓ Fine-tuned SBERT model loaded")

result = evaluate_all_fields(eval_model, label="SBERT Fine-Tuned — Fact+Issue")
all_results.append(result)
...
# ════════════════════════════════════════════════════════════
print("\n\n" + "="*60)
print("  SBERT FACT+ISSUE ABLATION FINAL RESULTS")
print("  Zero-Shot vs Fine-Tuned")
print("  Query: [FACT] [ISSUE] → Cited Case")
print("  Pool: 1 correct + 999 random | 9979 pools")
print("="*60)
summary_df = pd.DataFrame(all_results).set_index('Model')
print(summary_df.to_string())
summary_df.to_csv(f"{OUTPUT_DIR}/sbert_fact_issue_v1_results.csv")
print(f"\n✓ Results saved to: {OUTPUT_DIR}/sbert_fact_issue_v1_results.csv")
